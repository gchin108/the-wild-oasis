...
const StyledSalesChart = styled(DashboardBox)`
  grid-column: 1 / -1; //span from beginning to end of colomn
...
function SalesChart({ bookings, numDays }) {
  ...
  const allDates = eachDayOfInterval({
    start: subDays(new Date(), numDays - 1), //Subtract the specified number of days from the given date.
    end: new Date(),
  });

  //creating an object for each day in the array
  const data = allDates.map((date) => {
    return {
      label: format(date, "MMM dd"),
      totalSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at))) //dates checking, returns a boolean
